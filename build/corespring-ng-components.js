...
(function() {
  ...
  angular.module('cs.directives').directive('typeaheadTagger', function($timeout) {
    var definition;
    definition = {
      replace: true,
      template: "<span>\n<input type=\"text\"\n      change=\"onInputChange()\"\n         style=\"margin: 0 auto;\"\n         data-items=\"4\">\n         </span>",
      link: function(scope, element, attrs) {
        var addTag, onItemSelect, options, tagsModel;
        tagsModel = attrs['ttTagsModel'];
        addTag = function(item) {
          return scope.$apply(function() {
            if (!(scope[tagsModel] != null)) {
              scope[tagsModel] = [];
            }
            scope[tagsModel].push(item);
            return console.log("tags count: " + scope[tagsModel]);
          });
        };
        ...
        return $(element).find('input').typeahead(options);
      }
    };
    return definition;
  });
